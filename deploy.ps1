...
$remote = "cd ~/watan && git pull origin $branch && docker compose build --no-cache backend frontend && docker compose up -d backend frontend nginx && docker compose exec backend node dist/data-source.js migration:run"
ssh $server $remote
