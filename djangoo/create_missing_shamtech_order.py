...
# Create missing order in ShamTech
print("=== CREATING MISSING ORDER IN SHAMTECH ===")
...
# Create the missing order in ShamTech (forwarded from Al-Sham)
now = timezone.now()
order = ProductOrder.objects.create(
    ...
    user_identifier="test_user_123",  # Same as Al-Sham order
    quantity=1,
    price=10.00,
    status='pending',
    mode='MANUAL',
    created_at=now,
    notes="Forwarded from Al-Sham - test_user_123",
    external_order_id=f"stub-{uuid.uuid4()}",
    chain_path=json.dumps(["diana", "shamtech"])
)

print(f"\nCreated missing order in ShamTech:")
print(f"  ID: {order.id}")
...
print("\nSUCCESS: Missing order created in ShamTech!")
print("Now the order should appear in ShamTech interface")
