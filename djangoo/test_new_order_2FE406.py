...
print("="*80)
print("TESTING NEW ORDER 2FE406")
print("="*80)

# Find the new order 2FE406
order = ProductOrder.objects.filter(
    id__icontains='2FE406'
).first()

if not order:
    # Search by user_identifier
    orders = ProductOrder.objects.filter(
        user_identifier__icontains='2FE406'
    ).order_by('-created_at')[:5]
    
    if orders.exists():
        print(f"Found {orders.count()} orders containing 2FE406 in user_identifier:")
        for o in orders:
            print(f"  - {o.id} | {o.user_identifier} | {o.status}")
        order = orders.first()
    else:
        print("No order found containing 2FE406")
        exit(1)
