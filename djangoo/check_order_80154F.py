...
# Find the order 80154F
print("=== SEARCHING FOR ORDER 80154F ===")
try:
    # Try different search methods
    orders = ProductOrder.objects.filter(id__icontains='80154F')
    if orders.exists():
        order = orders.first()
        print(f"Found order by ID contains: {order.id}")
    else:
        # Try searching by external_order_id
        orders = ProductOrder.objects.filter(external_order_id__icontains='80154F')
        if orders.exists():
            order = orders.first()
