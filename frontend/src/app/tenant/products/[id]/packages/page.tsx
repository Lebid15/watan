...
export default function ProductPackages() {
  ...
  const isPackage = (v: any): v is PackageRow =>
    !!v && typeof v === 'object' && typeof v.id === 'string';

  const normalize = (raw: unknown): PackageRow[] => {
    if (Array.isArray(raw)) return raw.filter(isPackage);
    if (raw && typeof raw === 'object' && Array.isArray((raw as any).items))
      return (raw as any).items.filter(isPackage);
    return [];
  };
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const r = await api.get(`/products/${id}/packages`);
      ...
      <table className="w-full text-sm border border-border">
        ...
        <tbody>
          ...
          {!loading && rows.length === 0 && (
            <tr>
              <td className="p-4 text-center opacity-60" colSpan={3}>
                No packages
              </td>
            </tr>
          )}
