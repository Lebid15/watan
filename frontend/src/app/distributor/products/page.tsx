...
export default function CatalogImportPage(){
  ...
  useEffect(()=>{(async()=>{
    ...
    try {
      ...
      const raw = (r as any)?.data as any;
  ...
  const publish=()=>{ alert(`Queued ${selectedIds.length} products (stub)`); };

  return <div className="space-y-4">
    <h1 className="text-xl font-semibold">Catalog Import</h1>
    {loading && <div>Loading...</div>}
    {err && <div className="text-danger text-sm">Failed loading</div>}
    <div className="flex gap-2">
      <button disabled={!selectedIds.length} onClick={publish} className="btn btn-sm btn-primary">
        Queue Publish ({selectedIds.length})
      </button>
    </div>
    <table className="w-full text-sm border border-border">
      ...
      <tbody>
        ...
        {!loading && rows.length===0 && (
          <tr><td colSpan={4} className="p-4 text-center opacity-60">No products</td></tr>
        )}
      </tbody>
    </table>
  </div>;
