  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        const list: DistOrder[] = Array.isArray(raw)
          ? raw as DistOrder[]
          : Array.isArray(raw?.items)
          ? raw.items as DistOrder[]
          : [];
        setRows(list);
      } catch (e: any) {
        setErr(e);
          ...
          {rows.map((r) => (
            <tr key={r.id} className="border-t border-border">
              ...
              <td className="p-2 text-right">
                {(r.profitUsd || (r.sellUsd || 0) - (r.capitalUsd || 0)).toFixed(2)}
              </td>
            </tr>
          ))}
          {!rows.length && !loading && (
            <tr>
              <td colSpan={4} className="p-3 text-center text-xs opacity-60">
                No orders
              </td>
            </tr>
          )}
