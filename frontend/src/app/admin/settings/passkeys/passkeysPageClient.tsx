...
function normalizePasskeys(data: any): PasskeyItem[] {
  const root = Array.isArray(data)
    ? data
    : data?.items || data?.passkeys || data?.data || [];
...
export default function PasskeysPageClient() {
  ...
  const load = async () => {
    ...
    try {
      const res = await api.get<any>(
        API_ROUTES.auth.passkeys.list + '?_ts=' + Date.now(),
        { validateStatus: () => true }
      );
      try {
        console.log('[admin/passkeys] response', res.status, res.data);
      } catch {}
        ...
        if (!norm.length) {
          const keys =
            res && res.data && typeof res.data === 'object'
              ...
              : [];
            console.warn('[admin/passkeys] empty after normalize. keys=', keys);
        }
  ...
  return (
    <div className="admin-container py-4 max-w-3xl">
      ...
      <table className="w-full text-sm border border-border rounded overflow-hidden mt-4">
