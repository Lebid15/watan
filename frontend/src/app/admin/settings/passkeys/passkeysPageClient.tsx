...
export default function PasskeysPageClient() {
 ...
 api
 .get<any>(API_ROUTES.auth.passkeys.list + \?_ts=\ + Date.now(), {
 validateStatus: () => true,
 ...
 if (!errorShown) { setErrorShown(true); show(\يرجى تسجيل الدخول\); }
 router.push(\/login\);
 }
 })
 .catch((err) => {
 ...
 setTimeout(() => { setLoading(false); inFlight.current = false; }, remaining);
 });
 ...
 <h1 className=\text-xl font-bold mb-4\>مفاتيح المرور (الأمان)</h1>
 <p className=\text-sm text-text-secondary mb-6\>
 أدر مفاتيح المرور المرتبطة بحساب المالك. استخدم مفاتيح متعددة لأجهزة مختلفة.
 </p>
 ...
 {opError && <div className=\text-red-500 text-xs mt-[-12px] mb-4\>{opError}</div>}

 {loading && (
 <div className=\animate-pulse space-y-2\ aria-busy=\true\>
 ...
 <div className=\text-sm text-text-secondary transition-opacity duration-300 opacity-100\>
 لا توجد مفاتيح.
 </div>
 )}
 ...
 <td className=\p-2\>
 {pk.name || \Passkey\}{" \}
                <span className=\text-xs text-text-secondary\>#{pk.id?.slice?.(0, 8)}</span>
              </td>
              <td className=\p-2\>{pk.createdAt ? new Date(pk.createdAt).toLocaleString() : \\}</td>
              ...
              <td className=\p-2\>
                <button
                  ...
                  className=\text-red-600 hover:underline disabled:opacity-40\
                >
                  حذف
                </button>
              </td>
