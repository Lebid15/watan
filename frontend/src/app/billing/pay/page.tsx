...
export default function BillingPayPage(){
  ...
  return <div className="p-4 max-w-md space-y-4">
    ...
    {!methods.length && !suspended && <div className="bg-yellow-600 text-white p-3 rounded text-sm">لا توجد وسائل دفع مضافة بعد. الرجاء التوجه إلى صفحة وسائل الدفع لإضافة وسيلة.</div>}
    ...
    {res && <div className="text-green-500 text-sm">{t('billing.pay.success',{id:res.depositId})} <div className="text-xs opacity-70 mt-1">تم إرسال طلب الدفع، سيتم المراجعة والاعتماد. بعد الاعتماد يتجدّد الاشتراك للشهر التالي.</div></div>}
    {err && <ErrorMsg code={err?.response?.data?.code} />}
  </div>;
}
function ErrorMsg({code}:{code?:string}){ if(!code) return <div className="text-red-500 text-xs">خطأ غير متوقع</div>; const map:Record<string,string>={ INVALID_AMOUNT:'المبلغ غير صالح', METHOD_REQUIRED:'يجب اختيار وسيلة دفع', INVOICE_NOT_OPEN:'الفاتورة غير مفتوحة', TENANT_SUSPENDED:'تم تعليق المتجر بسبب تأخر الدفع' }; return <div className="text-red-500 text-xs" dir="ltr">{map[code]||code}</div>; }
