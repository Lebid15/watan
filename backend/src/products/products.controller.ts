...
export class ProductsController {
  ...
  @Body('type') type?: 'fixed' | 'unit',
  @Body('unitName') unitName?: string,
  // baseUnitPrice removed – per price group pricing only
  ...
  async getGroupPackageUnitPrice(
    ...
    return {
      ...
      groupId,
      packageId: pkg.id,
  unitPrice: pkg.type === 'unit' && pkg.product?.supportsCounter ? (row ? Number(row.price).toFixed(getPriceDecimals()) : null) : null,
