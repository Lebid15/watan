...
export class AuthController {
  ...
  @ApiBody({
    schema: {
      properties: { secret: { type: 'string' }, email: { type: 'string' }, password: { type: 'string' } },
      required: ['secret', 'email', 'password'],
    },
  })
  async bootstrapDeveloper(@Body() body: { secret: string; email: string; password: string }) {
    ...
    // إن وجد أي مطوّر سابقًا نمنع الإنشاء
  ...
  @ApiBody({
    schema: {
      properties: { secret: { type: 'string' }, email: { type: 'string' }, tenantId: { type: 'string' } },
  ...
  async assumeTenant(@Req() req: any, @Body() body: { tenantId: string }) {
    ...
    if (!(user.role === 'developer' || user.role === 'instance_owner')) {
      try {
        await this.audit.log('impersonation_denied', {
          actorUserId: user.id,
          meta: { tenantId: body.tenantId, reason: 'role_not_allowed' },
        });
      } catch {}
    ...
    try {
      await this.audit.log('impersonation_success', { actorUserId: user.id, targetTenantId: tenant.id });
    } catch {}
  ...
  async verifyEmail(@Body() body: { token: string }) {
    ...
    if (!token) {
      try {
        await this.audit.log('email_verify_fail', { meta: { reason: 'invalid_or_expired' } });
      } catch {}
    ...
    try {
      await this.audit.log('email_verify_success', {
        actorUserId: user.id,
        targetUserId: user.id,
        targetTenantId: user.tenantId ?? null,
      });
  ...
  async requestPasswordReset(@Body() body: { emailOrUsername: string; tenantCode?: string }, @Req() req: any) {
    ...
    if (tenantId) {
      user =
        (await this.usersRepo.findOne({ where: { email: body.emailOrUsername, tenantId } as any })) ||
  ...
  async resetPassword(@Body() body: { token: string; newPassword: string }) {
    ...
    if (!token) {
      try {
        await this.audit.log('password_reset_fail', { meta: { reason: 'invalid_or_expired' } });
      } catch {}
      throw new BadRequestException('Invalid token');
    }
    ...
    // تحديث كلمة المرور باستخدام argon2 لمواءمة المسارات الأخرى
    const argon2 = require('argon2');
    user.password = await argon2.hash(body.newPassword, { type: argon2.argon2id });
    await this.usersRepo.save(user);

    try {
      await this.audit.log('password_reset_success', {
        actorUserId: user.id,
        targetUserId: user.id,
        targetTenantId: user.tenantId ?? null,
      });
