...
export class ProductPackagePublicCodeScope20250830T2130 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    // 2. Create new unique index on (product_id, publicCode)
      // NOTE: Need to quote "publicCode" inside the WHERE clause for Postgres because original column
      // was created with camelCase (mixed-case) identifier; unquoted would fold to lower-case and fail.
    ...
    // Create a temporary table for backup (persisting in DB for manual retrieval)
      // Use CURRENT_TIMESTAMP (works on Postgres & SQLite) instead of sqlite-specific datetime('now')
