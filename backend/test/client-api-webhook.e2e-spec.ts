...
import { ClientApiWebhookWorker } from '../src/client-api/client-api-webhook.worker';
import { ClientApiWebhookAdminController } from '../src/client-api/client-api-webhook.admin.controller';
...
describe('Client API Webhook Outbox (e2e)', () => {
  ...
  async function ensureTenant(tenantId: string) {
    // minimal tenant row (columns: id, name, code, ownerUserId, isActive, createdAt, updatedAt)
    await ds.query(`INSERT INTO tenant (id, name, code, "ownerUserId", "isActive", createdAt, updatedAt) VALUES (?,?,?,?,?,?,?)`, [tenantId, 'Webhook Tenant', tenantId.slice(0,12), null, 1, new Date().toISOString(), new Date().toISOString()]);
  }
  ...
  it('enqueue on status change', async () => {
    ...
    if (typeof rows[0].payload_json === 'string') {
      try { payload = JSON.parse(rows[0].payload_json); } catch { payload = rows[0].payload_json; }
    } else {
  ...
  it('success path sets succeeded + response_code', async () => {
    ...
    await productsService.updateOrderStatus((order as any).id, 'approved');
    await forceDueAll();
  const worker = app.get(ClientApiWebhookWorker);
    await worker.tick();
  ...
  await app.get(ClientApiWebhookAdminController).retry({ user: { tenantId: (user as any).tenantId } } as any, id); // direct call
  ...
  (global as any).__FETCH_SCENARIO__ = 'success';
    ...
    // redeliver
  const redeliverRes: any = await app.get(ClientApiWebhookAdminController).redeliver({ user: { tenantId: (user as any).tenantId } } as any, id);
    ...
    // mark dead
  await app.get(ClientApiWebhookAdminController).markDead({ user: { tenantId: (user as any).tenantId } } as any, newRow.id);
